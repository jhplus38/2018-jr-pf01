 // hori 첫번째 방법 (한방향으로만 흐름)
/*
var now = 1;
var end = $(".slide").length;
$(".banner_wrap").append($(".slide").eq(0).clone());
//$(".slide").eq(0).clone().appendTo($(".banner_wrap")); //clone(복제)
$(".slide").each(function(i){
	$(this).css({"left":(i*100)+"%"});
});
ani(); //최초의 ani
function ani() {
	$(".banner_wrap").delay(2000).animate({"left":-(now*100)+"%"}, 1000, function(){ //-now*100만큼 움직여라
		if(now == end) {
...
function init() {
	$(".banner_wrap").css({"left":0});
	$(".slide").hide(0); //.slide 숨김
	$(".slide").eq(now).css({"left":0}).show(0); //.slide 중 나타날 now번째 배너
	if(now == 0) {
...
} //now에 따라 해당번호의 이미지부터 3개 연달아 나옴
function ani() {
	init(); //ani들어가는 순간 init
	$(".banner_wrap").stop().animate({"left":(100*dir)+"%"}, speed, function(){
		if(dir == -1) { //direction이 -1이면 정방향으로 이동
			if(now == end) now = 0;
			else now++;
		}
		else {
			if(now == 0) now = end;
			else now--; //now가 0이면 그 다음은 end니까 -1해줌
		}
	});
}
...
var SlideHori = (function(){ //즉시 실행 함수
	function SlideHori(container, slides, options) {
		...
		this.init(obj);
		this.interval = setInterval(this.ani, this.options.delay, obj); //this.ani실행하고, this.delay시간
